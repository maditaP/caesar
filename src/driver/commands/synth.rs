...
use crate::synthesis::cegis::{run_cegis_loop, CegisProof, get_synth_functions, CegisData};
...
use crate::synthesis::rec_functions::{run_soundness_check, SoundnessOutcome};
...
fn synth_main(
    ...
    // Unified refinement counter.
    // - KInduction: this is k, starting from the annotation value.
    // - Degree: this is the polynomial degree, starting from --degree.
    // - Fixed/Variable: this is the number of piecewise intervals, starting from 1.
    ...
    // Tracks which items were definitively proven across all outer iterations.
    let mut proven_item_names: IndexSet<String> = IndexSet::new();
    let mut stats = SynthStats::new();
    // Current fuel for the guarded encoding; incremented on each soundness-check failure
    // so that a wider defined range is exposed to CEGIS on the next attempt.
    let mut current_fuel: usize = options.opt_options.max_fuel;

    // Collect item names for sequential per-item synthesis. Names are stable
    // across re-parses of the same source (Item::flat_map preserves them).
        ...
        'refinement: while refinement_iter <= max_refinement_iter {
            ...
            if options.synth_options.syn_verbose {
                match refinement_mode {
                    RefinementMode::Degree => println!(
                        "Degree {refinement_iter}"
                    ),
                    RefinementMode::None => println!("Single CEGIS attempt"),
                    RefinementMode::Variable | RefinementMode::Fixed => println!(
                        "{refinement_iter} interval(s)"
                    ),
                    RefinementMode::KInduction => println!(
                        "Iteration {refinement_iter} (k-induction, k={refinement_iter})"
                    ),
                }
            }
            ...
            // Collect the inv_call argument from each @k_induction(k, inv_call) annotation
            // before the module is consumed by apply_encodings / the item loop.
            // Used to reprint the updated annotation in the synthesized output.
            let k_induction_calls = (refinement_mode == RefinementMode::KInduction)
                .then(|| collect_k_induction_calls(&mut module))
                .unwrap_or_default();
            ...
            for item in module.items {
                ...
                // For rec_functions: clone the unmodified task for the soundness check,
                // then apply the visitor to the synthesis copy's block.
                ...
                // compute original VC now while &mut tcx is still available; used for soundness check
                ...
                let (instantiated_templates, range_constraints) =
                    run_cegis_for_item(
                        &tcx,
                        &depgraph,
                        options,
...
fn run_cegis_for_item(
    ...
    current_fuel: usize,
) -> Result<(Option<Vec<(Ident, Expr)>>, Vec<Expr>), CaesarError> {
    ...
    let tvar_ident_set: IndexSet<Ident> = tvars.iter().map(|(id, _)| id.clone()).collect();
    ...
    let range_constraints: Vec<Expr> =
        collect_ranges_from_decls(&tcx.declarations.borrow())
            .iter()
            .map(|(ident, (range, ty))| {
                create_range_constraint(ident.clone(), range, ty.clone())
            })
            .collect();
