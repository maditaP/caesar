...
pub fn get_synth_functions<'ctx>(
    ...
    uninterps.functions()
        .iter()
        .filter_map(|(id, f)| f.syn.then_some((id.clone(), f)))
        .collect()
}
...
pub fn verify_candidate<'smt, 'ctx>(
    ...
    let bool_vc_task = BoolVcProveTask {
        quant_vc: data.vc_expr.clone(),
        vc: bool_vc_inst,
    };
    let smt_task = SmtVcProveTask::translate(bool_vc_task, translate);
    let result = smt_task.no_slice_run_solver(
        options,
        limits_ref,
        ctx,
        translate,
    ...
    let kind = match result.prove_result {
        ProveResult::Proof => VerifyOutcomeKind::Proof,
        ProveResult::Counterexample => match result.into_model() {
            Some(model) => {
                let cex_mapping =
                    create_subst_mapping(&data.pvar_idents, &model, translate);
...
pub fn synthesize_from_cex<'smt, 'ctx>(
    ...
    let bool_vc_inst = subst_from_mapping(cex_mapping, &data.boolean_vc.vc, limits_ref, translate.ctx)?;
    ...
    get_model_for_constraints(
        ...
        cex_task,
        translate,
        &data.tvars.iter().map(|(id, _)| id.clone()).collect(),
        ctx,
    )
}
...
pub fn run_cegis_loop<'smt, 'ctx>(
    ...
    loop {
        ...
        let outcome = verify_candidate(data, &tvar_mapping, options, limits_ref, ctx, translate)?;
        ...
        match outcome.kind {
            ...
            VerifyOutcomeKind::Counterexample(cex_mapping) => {
                ...
                if let Some(ref prev) = prev_cex_mapping {
                    let dist: BigRational = cex_mapping
                        .iter()
                        .filter(|(id, _)| !data.tvars.iter().any(|(tid, _)| tid == *id))
                        .filter_map(|(id, val)| prev.get(id).map(|prev_val| expr_lit_distance(val, prev_val)))
                        .sum();
                    if dist <= BigRational::from_integer(CLOSE_CEX_DISTANCE_THRESHOLD.into()) {
                        close_cex_count += 1;
                        if options.synth_options.syn_verbose {
                            println!("Close CEX (distance={dist}), total count: {close_cex_count}");
                        }
