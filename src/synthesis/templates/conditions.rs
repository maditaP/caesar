        ...
        for mapping in &mappings {
            if vars.iter().all(|v| mapping.contains_key(v)) {
                ...
                for (pvar, fvar_expr) in mapping {
                    pvar_to_fvar_map.insert(pvar.clone(), fvar_expr.clone());
                }
                continue 'bools;
            }
        }
...
// Normalises `(a - c1) == c2` → `a == (c1 + c2)` for UInt literals, so that
...
pub(super) fn normalize_condition(expr: &Expr) -> Expr {
    ...
    Shared::new(ExprData {
        kind: ExprKind::Binary(op.clone(), inner_lhs.clone(), new_rhs),
        ty: expr.ty.clone(),
        span: expr.span,
    })
}
            ...
            for member in members {
                ...
                if prover.check_sat() == SatResult::Sat {
                    let new_guard = if neg_is_sat {
                        ...
                        tracing::trace!("implied exclusive member at depth {idx}, not added to guard");
                        partial_guard.clone()
                    };
                    num_sat_checks += explore_condition_groups(
                        idx + 1,
