...
/// Implements static hyperrectangle refinement: partitions each range-annotated variable into
/// `split_count` equal-width intervals and returns one Boolean predicate per Cartesian-product cell.
/// Boolean variables in `bool_vars` are always split into exactly two regions: `var` and `!var`.
pub fn get_fix_region_splits(
    ...
    for (var, range) in ranged_vars {
        ...
        for i in 1..=(num_parts) {
            ...
            let lo_expr =
                builder.signed_frac_lit(BigRational::from_integer(BigInt::from(part_lo)));
            let hi_expr =
                builder.signed_frac_lit(BigRational::from_integer(BigInt::from(part_hi)));
...
/// Implements dynamic hyperrectangle refinement: introduces fresh synthesized boundary template
/// variables and splits each formal parameter into `split_count` half-space regions.
/// Boolean variables in `bool_vars` are always split into exactly two regions: `var` and `!var`.
/// No range annotations are required. Returns `[true]` when there are no variables to split.
pub fn get_variable_region_splits(
    ...
    for (var_idx, var) in fvars.iter().enumerate().take(if split_count <= 1 { 0 } else { fvars.len() }) {
        let real_var = if matches!(var.ty, Some(TyKind::Real)) {
            var.clone()
        } else {
            builder.cast(TyKind::Real, var.clone())
            ...
            acc.iter()
                .flat_map(|prefix| {
                    list.iter().map(move |item| {
                        and_simplify(builder, prefix.clone(), item.clone())
                    })
                })
                .collect()
        })
