use crate::{
    ...
    smt::{
        ...
        uninterpreted::FuncEntry,
        DepConfig, SmtCtx,
    },
    synthesis::{
        cegis::get_synth_functions,
        report::SynthStats,
    },
    tyctx::TyCtx,
};
...
impl<'smt, 'ctx> FunctionInliner<'smt, 'ctx> {
    pub fn new(
        ...
        Self { target, entry, body, tcx, inlining_stack: Vec::new() }
    }
}
...
use super::{build_template_expression, RefinementState, TemplateEntry, TemplateResult};
...
pub fn build_and_inline_templates<'smt, 'ctx>(
...
) -> Result<(Vec<TemplateEntry>, Vec<(Ident, TyKind)>), CaesarError> {
    ...
    for (synth_name, synth_val) in synth_funcs.iter() {
        ...
        Unfolder::new(limits_ref.clone(), smt_ctx).visit_expr(&mut vc_unfolded)?;
        ...
        let TemplateResult {
            ...
            &mut builder,
            tcx,
            refinement.effective_split_count,
            translate,
            ctx,
            limits_ref.clone(),
            &vc_unfolded,
            options.synth_options.bare_template,
        ...
        let ctx_local = Context::new(&z3::Config::default());
        ...
        Unfolder::new(limits_ref.clone(), &smt_ctx_local).visit_expr(&mut template_expr)?;
        ...
        if options.synth_options.print_template {
            println!("template for `{}`: {}", synth_name, remove_casts(&template_expr));
        }
        templates.push(TemplateEntry {
            synth_name: synth_name.clone(),
            expr: template_expr,
            guards_before_pruning,
            guards_after_pruning,
            loop_mode,
    ...
    for i in 0..templates.len() {
        let other_templates: Vec<(Ident, Expr)> = templates
            ...
            .enumerate()
            .filter(|(j, _)| *j != i)
            .map(|(_, t)| (t.synth_name.clone(), t.expr.clone()))
            .collect();
        ...
        Unfolder::new(limits_ref.clone(), &smt_ctx_local).visit_expr(&mut templates[i].expr)?;
    }
    ...
    *vc_is_valid = lower_quant_prove_task(options, limits_ref, tcx, name, vc_expr.clone())?;
