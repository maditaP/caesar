//! Intrinsic for unchecked casting of a signed expression to its non-negative
//! counterpart (Int → UInt, Real → UReal).  The caller is responsible for
//! ensuring the argument is actually non-negative; the non-negativity
//! obligation is tracked separately and added to the verification condition.
...
/// An unchecked cast from a signed numeric type (Int/Real) to the corresponding
/// non-negative type (UInt/UReal).  The argument is assumed to be ≥ 0; this
/// assumption must be discharged externally as part of the verification
/// condition.
#[derive(Debug)]
